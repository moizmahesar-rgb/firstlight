...
def solve(hour, storage_mwh):
    ...
    closeout_charge = max(rt_lmp - strike_price, 0)
    tmnsr_revenue = MW * (tmnsr_price - closeout_charge)

    options = []

    options.append((solve(hour + 1, storage_mwh), 'IDLE', storage_mwh))
    options.append((tmnsr_revenue + solve(hour + 1, storage_mwh), 'TMNSR', storage_mwh))

    if storage_mwh + PUMP_ADDITION <= MAX_STORAGE:
        pump_cost = MW * da_price
        next_storage = storage_mwh + PUMP_ADDITION
        options.append((-pump_cost + solve(hour + 1, next_storage), 'PUMP', next_storage))

    if storage_mwh >= GEN_REQUIREMENT:
        gen_revenue = MW * da_price
        next_storage = storage_mwh - GEN_REQUIREMENT
        options.append((gen_revenue + solve(hour + 1, next_storage), 'GENERATE', next_storage))
...
for hour in range(24):
    ...
    closeout_charge = max(rt_lmp - strike_price, 0)
    net_tmnsr_price = tmnsr_price - closeout_charge
    ...
    print(f"Hour {hour+1:2d}: {action:8s} | Revenue: ${hourly_revenue:>11,.0f} | Storage: {starting_storage:>7.0f} → {ending_storage:>7.0f} MWh")
